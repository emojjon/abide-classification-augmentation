...
model.add(layers.Input(shape=vol_size))
...
lcls = locals()
if 'X_test' in lcls and 'Y_test' in lcls:
    X = lcls['X_test']
    Y = lcls['Y_test']
else:
    X = X_val
    Y = Y_val
test_stats = np.empty((X.shape[0], 3))
Y_pred = np.empty_like(Y)
row = 0
chunk = 32 # Should memory run out, this could be lowered.
while row < X.shape[0]:
    Y_pred[row : row + chunk, None] = model.predict_on_batch(X[row : row + chunk, :, :, :, :])
    row += chunk
sample_loss = test_loss(Y, Y_pred)
test_stats[:, 0] = Y
test_stats[:, 1] = Y_pred
test_stats[:, 2] = sample_loss.numpy()
np.save(os.path.join(res_path, f'test_stats_{mid}.npy'), test_stats)
...
with open(os.path.join(res_path, f'report_{mid}.txt'), "a") as report:
    ...
    correct_tests = test_stats[abs(test_stats[:, 0] - test_stats[:, 1]) < 0.5]
    print(f"Test accuracy: {correct_tests.shape[0] / test_stats.shape[0]:.4f} \n"
          f"Test loss: {test_stats[:, 2].mean():.4f}", file = report)
